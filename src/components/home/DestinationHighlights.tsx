        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {destinations.map((destination) => (
            <article key={destination.slug} className="card overflow-hidden">
              <ResponsiveImage src={destination.coverImage || destination.image} alt={destination.imageAlt || destination.name} title={destination.name} subtitle={destination.region} className="h-44 w-full rounded-none" sizes="(max-width: 768px) 100vw, 33vw" />
              <div className="p-5">
